...
parser.add_argument('--n-gramm', default=1, type=int)
...
def performance(args, SNR, net):
    ...
    if args.n_gramm == 1:
        ...
        bleu_score_1gram = BleuScore(0, 1, 0, 0)
    elif args.n_gramm == 3:
        bleu_score_1gram = BleuScore(0, 0, 1, 0)
    elif args.n_gramm == 4:
...
if __name__ == '__main__':
    ...
    print(bleu_score)
    file = open('bleu_score.txt', 'a')
    file.write("\n" + args.channel + "\n")
    file.write(str(args.n_gramm) + "\n")
    file.write(' '.join(str(x) for x in bleu_score) + '\n')
